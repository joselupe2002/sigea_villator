...
<?php session_start(); if (($_SESSION['inicio']==1)) {
	...
	class PDF extends FPDF {
		...
		utf8_decode($data[0]["MAPA"]).utf8_decode(' aprobó los 8 niveles, de los cursos de capacitación para la '.
		'comprensión de artículos técnico-científicos en una lengua extranjera.'),0,'J', false);
		$pdf->Ln(5);
		...
		$pdf->MultiCell(0,8,utf8_decode('Se extiende la presente en la ciudad de '.$dataGen[0]["inst_extiende"].', a los '.$fechaof.', para los fines legales que convengan al interesado.'),0,'J', false);
		$pdf->Ln(5);
		...
		if ($_GET["tipo"]=='2') {
	      ...
	      <?php 
					foreach($dataProf as $rowdes)
					{
						...
						$res=$miutil->enviarCorreo($rowdes[2],'SIGEA:ITSSMO Asignación de Comisión '.$data[0]["COMI_ID"].$data[0]["COMI_ACTIVIDAD"],
