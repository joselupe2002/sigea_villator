...
<?php session_start(); 
	...
	class PDF extends FPDF {
        ...
        function parseVar($key='',$value='') {
			...
			{
				$miConex = new Conexion();
				$resultado=$miConex->getConsulta("SQLite","SELECT * from INSTITUCIONES where _INSTITUCION='ITSP'");
				foreach ($resultado as $row) {
					$data[] = $row;
				}
				return $data;
		...
		*/
		...
		if ($_GET["tipo"]=='2') {
	      ...
	      <?php 
						...
						$res=$miutil->enviarCorreo($data[0]["CORREO"],'SIGEA:ITSP CONSTANCIA '.$data[0]["EVENTOPRIND"].$data[0]["EVENTOD"],
