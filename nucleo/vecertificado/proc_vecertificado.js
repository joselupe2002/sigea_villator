...
function imprimeCertificadoM(modulo,usuario,institucion, campus,essuper){
	table = $("#G_"+modulo).DataTable();
	if (table.rows('.selected').data().length>0) {		
		elid=table.rows('.selected').data()[0][0];
		enlace="nucleo/vecertificado/certificadoM.php?matricula="+table.rows('.selected').data()[0]["MATRICULA"]+
		"&folio="+table.rows('.selected').data()[0]["FOLIO"];
